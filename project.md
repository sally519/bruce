...
## 七、DocNode 子图详细设计（已实现）

### 7.1 功能概述

DocNode 是工作流的第一个节点，负责根据需求文档生成 PRD（产品需求文档）。
...
|------|--------|----------|
| PDF | .pdf | pypdf 提取文本 |
| Word | .docx, .doc | python-docx 提取文本 |
| 图片 | .png, .jpg, .jpeg | OCR (pytesseract) |
| Markdown | .md | 直接读取 |
...
```
┌─────────────────┐
│   scan_files    │  扫描 doc/requestion 目录
└────────┬────────┘
         ↓
┌─────────────────┐
...
└────────┬────────┘
         ↓
┌─────────────────┐
│  generate_prd   │  DocAgent 生成 PRD 草稿
│                 │  同时检测逻辑完整性
└────────┬────────┘
         ↓
┌─────────────────┐
│ check_user_input│  检测是否需要用户介入
└────────┬────────┘
         ↓
    ┌────┴────┐
    │需要介入？ │
...
**触发条件**：
DocAgent 在生成 PRD 过程中，如果发现以下问题会触发用户介入：
1. 需求描述不完整 - 关键功能模块有遗漏
2. 需求逻辑不自洽 - 不同部分描述存在冲突
3. 业务流程不闭环 - 有缺失的环节

**介入方式**：
1. **文本直接输入**：用户可以直接输入补充内容
2. **文件传入**：用户可以提供补充文件（支持与原始文件相同格式）

**设计原则**：
- 尽可能一次性列出所有需要补充的问题，减少打断次数
- 只有在确实存在逻辑问题时才打断，避免过度干扰
- 用户补充后可继续生成，如仍不完整可再次打断

### 7.5 Agent 设计

**DocAgent（需求生成 Agent）**
- 角色：资深产品经理
- 职责：根据输入需求内容生成完整 PRD，同时检测逻辑完整性
- 特殊职责：在 PRD 末尾标注需要用户补充的内容（如有）
- 输出：PRD 草稿 + 是否需要用户介入的判断

**ReviewAgent（专家审核 Agent）**
- 角色：产品专家评审委员会
- 职责：审核 PRD 质量，给出评分和修改意见
- 评分标准：
  - 90-100 分：优秀，直接通过
  - 70-89 分：良好，有小问题
  - 60-69 分：一般，需较大修改
...
1. **评分阈值**：≥90 分才能通过
2. **强制审核**：对于超过 100 字的需求内容，专家至少提出一次修改建议
3. **迭代循环**：不通过时返回 DocAgent 重新生成，直到通过
4. **用户介入优先**：用户介入流程优先于审核流程

### 7.7 文件路径

- **输入目录**：`doc/requestion/` - 放置需求文档
- **输出目录**：`doc/output/` - 生成最终 PRD

### 7.8 大模型配置

使用 `.env` 中配置的 MiniMax 大模型：
